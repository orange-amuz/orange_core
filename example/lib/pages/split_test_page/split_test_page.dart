...
class _SplitTestPageState extends State<SplitTestPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (timestamp) {
        ...
        setState(() {
          circles.addAll([defaultCircle]);
          parentCircles.addAll([defaultCircle]);
  ...
  Future<void> onTapUp(
    ...
    parentCircles.add(bigCircle);
    parentCircles.add(smallCircle);
  ...
  List<CircleModel> getInscriptions(
    ...
    // final centers = getCenters(c1, c2, newRadius);
    ...
    // final indexP = dots.indexWhere((element) => isUnique(element, newCircleP));
    // final indexN = dots.indexWhere((element) => isUnique(element, newCircleN));

    // if (indexP == -1) {
    //   dots.add(CircleModel(offset: newCircleP.offset, diameter: 3));
    // }

    // if (indexN == -1) {
    //   dots.add(CircleModel(offset: newCircleN.offset, diameter: 3));
    // }
  ...
  Widget build(BuildContext context) {
                  ...
                  children: [
                    Stack(
                      children: List.generate(
                        parentCircles.length,
                        (circleIndex) {
                          final circle = parentCircles.elementAt(circleIndex);

                          final top = defaultCircle.radius +
                              circle.offset.dy -
                              circle.radius;
                          final left = defaultCircle.radius +
                              circle.offset.dx -
                              circle.radius;
                    ...
                    Stack(
                      children: List.generate(
                        circles.length,
                        (circleIndex) {
                          final circle = circles.elementAt(circleIndex);

                          final parentOffset = Offset(
                            defaultCircle.radius -
                                (-circle.offset.dx + circle.radius),
                            defaultCircle.radius -
                                (-circle.offset.dy + circle.radius),
                          );
                          ...
                          return Positioned(
                            ...
                            child: ClipRRect(
                              borderRadius:
                                  BorderRadius.circular(circle.diameter),
                              child: Container(
                                width: circle.diameter,
                                height: circle.diameter,
                                decoration: BoxDecoration(
                    ...
                    Stack(
                      children: List.generate(
                        dots.length,
                        (circleIndex) {
                          final circle = dots.elementAt(circleIndex);
    ...
    //       final defaultDiameter = constraints.maxWidth - 16;
    ...
    //               circles.length,
    //               (index) {
    //                 final circle = circles.elementAt(index);

    //                 final diameter = circle.diameter;
    //                 final left = circle.offset.dx - diameter / 2;
    //                 final top = circle.offset.dy - diameter / 2;
    ...
    //             )..addAll([
    //                 Positioned(
    //                   left: defaultCircle.offset.dx - 1,
    //                   top: defaultCircle.offset.dy - 1,
    //                   child: Container(
    //                     width: 2,
    //                     height: 2,
    //                     color: Colors.yellow,
    ...
    //                 ),
    //                 Positioned(
    //                   left: smallOffset?.dx ?? 0,
    //                   top: smallOffset?.dy ?? 0,
    //                   child: Container(
    //                     width: 2,
    //                     height: 2,
    //                     color: Colors.purple,
...
class CircleModel {
  ...
  Rect get boundingBox => Rect.fromCircle(
        center: Offset(
          offset.dx,
          offset.dy,
        ),
...
class RTree {
  ...
  void insert(CircleModel circle) {
    circles.add(circle);
  }

  bool overlaps(Rect rect) {
    for (final circle in circles) {
      if (circle.boundingBox.overlaps(rect)) {
        return true;
      }
    }
